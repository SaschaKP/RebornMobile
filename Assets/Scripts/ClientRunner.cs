...
public class ClientRunner : MonoBehaviour
{
    ...
    {
	    CUOEnviroment.ExecutablePath = ServerConfiguration.GetPathToSaveFiles();
	    ...
	    Settings.GlobalSettings.IP = ServerConfiguration.UoServerUrl;
	    Settings.GlobalSettings.Port = ushort.Parse(ServerConfiguration.UoServerPort);
	    
	    //Reset static encryption type variable
	    EncryptionHelper.Type = ENCRYPTION_TYPE.NONE;
	    Settings.GlobalSettings.Encryption = (byte) (ServerConfiguration.UseEncryption ? 1 : 0);
	    ...
	    Settings.GlobalSettings.ShardType = 0;
	    ...
	    if (ClientVersionHelper.IsClientVersionValid(ServerConfiguration.ClientVersion, out var clientVersion))
	    {
	    ...
	    Settings.GlobalSettings.ClientVersion = ServerConfiguration.ClientVersion;
	    
	    if (Application.isMobilePlatform == false && string.IsNullOrEmpty(ServerConfiguration.ClientPathForUnityEditor) == false)
	    {
		    Settings.GlobalSettings.UltimaOnlineDirectory = ServerConfiguration.ClientPathForUnityEditor;
	    }
	    else
	    {
		    Settings.GlobalSettings.UltimaOnlineDirectory = ServerConfiguration.GetPathToSaveFiles();
	    }
    ...
    private void ApplyScalingFactor()
    {
		...
		else if (UserPreferences.ScaleSize.CurrentValue != (int)PreferenceEnums.ScaleSizes.Default)
		{
