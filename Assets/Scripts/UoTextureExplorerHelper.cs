...
public static class UoTextureExplorerHelper
{
    ...
    public static void CreateLandTileTextureAtlas()
    {
        ...
        for (int i = 0; i < 65536; i++)
        {
            ...
            if (entry.Length == 0)
            {
                if (range.y >= range.x)
                {
                    DebugInternal.Log($"Range {range.x}-{range.y}");
        ...
        DebugInternal.Log($"Total number of valid ushort keys {runningTotal}");
        DebugInternal.Log($"Number of duplicates {numberOfDuplicates}");
        DebugInternal.Log($"Duplicate indexes range from {duplicateIndexes[0]} to {duplicateIndexes[duplicateIndexes.Count - 1]}");
        DebugInternal.Log($"Number of unique tiles {runningTotal - numberOfDuplicates}");
    }
    ...
    public static void MakeAtlas(int rangeMin, int rangeMax)
    {
        ...
        {
            DebugInternal.LogWarning($"Can't fit all textures onto atlas. Total pixels {totalPixels} vs atlas pixels {atlasSize * atlasSize}");
        }
